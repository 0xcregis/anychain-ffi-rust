...
const anychain_ffi_lib = new ffi.Library(
    "../../target/release/libanychain_ffi_lib.dylib",
    {
        ...
        'build_raw_transaction': ['string', ['string', 'int']],
        'raw_transaction_signing_hashes': ['string', ['int', 'string', 'string']],
...
let address = payload(anychain_ffi_lib.create_address(xpub, 0, 0, 1))
assert('1EjwNvzypjwuYuqZFcX3wSfPpPf5sNG9b5', address)


let tx_params = JSON.stringify({
    "inputs": [
        {
...
let raw_tx = payload(anychain_ffi_lib.build_raw_transaction(tx_params, 0))
assert(
    '',
    raw_tx
)
...
let messages = payload(anychain_ffi_lib.raw_transaction_signing_hashes(0, raw_tx, reserved))
assert(
    [
...
let signed_tx = payload(anychain_ffi_lib.insert_signatures(sigs, 0, raw_tx, reserved))
assert(
...
let address_types = JSON.stringify(["p2pkh", "p2pkh"])
let bandwidth = payload(anychain_ffi_lib.estimate_bandwidth(tx_params, 0, address_types))
